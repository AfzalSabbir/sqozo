<?php
define('DELTA_THEME_VERSION', '1.2.0');
...
function del_extra_add_admin( $catId = null, $item_id = null ){
  ...
  if(defined('OC_ADMIN') && OC_ADMIN === true) {
    if($item_id > 0) {
      ...
      echo '<label class="control-label" for="sTransaction">' . __('Transaction', 'delta') . '</label>';
      echo '<div class="controls">' . del_simple_transaction(true, $item_id <> '' ? $item_id : false) . '</div>';
      echo '</div>';

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sCondition">' . __('Condition', 'delta') . '</label>';
      echo '<div class="controls">' . del_simple_condition(true, $item_id <> '' ? $item_id : false) . '</div>';
      echo '</div>';

      if(!method_exists('ItemForm', 'contact_phone_text')) {
        echo '<div class="control-group">';
        echo '<label class="control-label" for="sPhone">' . __('Phone', 'delta') . '</label>';
...
AdminMenu::newInstance()->add_submenu_divider('delta_menu', __('Theme Settings', 'delta'), 'delta_submenu');
...
if( !function_exists('logo_header') ) {
  function logo_header() {
    ...
    if(file_exists(WebThemes::newInstance()->getCurrentThemePath() . 'images/logo.jpg')) {
      return $html;
    } else if( del_param('default_logo') && (file_exists( WebThemes::newInstance()->getCurrentThemePath() . 'images/default-logo.jpg')) ) {
